class Tile:
    def __init__(self, tile, tile_id = 0):#tile_id = 0
        self.tile = tile
        self.id = tile_id
...
def check(order, tile, edge_size):
...
def search(target, pic, print_to_terminal):
...
def search_pic(target, pic, print_to_terminal):
    for r in reassemble:
        r(pic)
...
def part2(tiles):
    ...
    return search_pic(['#'], pic, False) - search_pic(sea_monster, pic, False) * 15
